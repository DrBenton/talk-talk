...
class Application implements ApplicationInterface
{
    ...
    public function addRestResource($methods, $urlPattern, $callback)
    {
        $this->get('logger')
            ->debug(sprintf('Action with URL "%s" added to Silex router.', $urlPattern));

        $silexRoute = call_user_func(array($this->silexApp, 'match'), $urlPattern, $callback);
        $silexRoute->method($methods);
    ...
    public function run()
    {
        ...
        $this->get('logger')
            ->debug('silexApp#run() ; URL=' . $this->request->getPathInfo());
