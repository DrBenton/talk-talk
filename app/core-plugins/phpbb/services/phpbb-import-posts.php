...
$app->vars['phpbb.import.posts.nb_items_per_batch'] = 200;

$app->defineFunction(
    'phpbb.import.posts.metadata',
    function () use ($app) {
        $ret = array();
        $ret['nbItemsPerBatch'] = $app->vars['phpbb.import.posts.nb_items_per_batch'];
...
$app->defineFunction(
    'phpbb.import.posts.trigger_batch',
    function ($nbToImport, $from) use ($app) {
        ...
        $topicsIdsMapping = $app->get('session')->get('phpbb.import.topics.ids_mapping');
        ...
        foreach ($phpBbPosts as $phpBbPost) {
            ...
            $app->exec('phpbb.import.add_provider_data', $talkTalkPost, $phpBbPost->post_id);
            $talkTalkPost->save();
